# -*- coding: utf-8 -*-
# @Time    : 2022/12/1 13:17
# @Author  : Lei Gao
# @FileName: repulsion_oriented_reppoints_head.py
# @Software: PyCharm
import math
...
from .utils import levels_to_images, perpendicular_dist
...
class RepulsionOrientedRepPointsHead(BaseDenseHead):
    ...
    def __init__(self,
                 ...
                 repulsion_loss=dict(type='RepulsionReppointsLoss', weight=1.0, part_weight=[1.0, 0.5, 0.5]),
                 conv_cfg=None,
                 norm_cfg=None,
                 train_cfg=None,
                 test_cfg=None,
                 ...
                 init_cfg=dict(
                     ...
                     override=dict(
                         ...
                         std=0.01,
                         bias_prob=0.01)),
                 centerness_alpha=4.0,
                 **kwargs):
        super(RepulsionOrientedRepPointsHead, self).__init__(init_cfg)
        ...
        # repeat behind each element [x1, ..., x2, ..., x3, ...]
        dcn_base_y = np.repeat(dcn_base, self.dcn_kernel)
        # repeat whole array at the tile [x1, x2, x3, x1, x2, x3, ...]
    ...
    def offset_to_pts(self, center_list, pred_list):
        ...
        pts_list = []
        for i_lvl, _ in enumerate(self.point_strides):  # 当前特征图的尺度
            pts_lvl = []
            for i_img, _ in enumerate(center_list):  # 图片序号
                pts_center = center_list[i_img][i_lvl][:, :2].repeat(
                    1, self.num_points)  # 获取当前图片的当前尺度的坐标信息
                pts_shift = pred_list[i_lvl][i_img]  # 获取当前生成的9个点的偏移信息
                yx_pts_shift = pts_shift.permute(1, 2, 0).view(
                    -1, 2 * self.num_points)  # 调整维度
                y_pts_shift = yx_pts_shift[..., 0::2]
                x_pts_shift = yx_pts_shift[..., 1::2]
                xy_pts_shift = torch.stack([x_pts_shift, y_pts_shift], -1)
                xy_pts_shift = xy_pts_shift.view(*yx_pts_shift.shape[:-1], -1)  # 以上几步为调整xy坐标的顺序
                pts = xy_pts_shift * self.point_strides[i_lvl] + pts_center  # 偏移比例*当前每个点的尺度+中心点坐标=9个点坐标
                pts_lvl.append(pts)
    ...
    def get_adaptive_points_feature(self, features, pt_locations, stride):
        ...
        sampled_features = torch.zeros([
            pt_locations.shape[0],  # batch_size
            features.size(1),  # channel
            pt_locations.size(1),  # num_points = 9
            pt_locations.size(2)  # num_coordinates = 2
    ...
    def centerness_quality(self, pos_center, pos_pred_polygons):
        """
        compute centerness of predicted polygons
        Args:
            pos_center: feature map center points
            pos_pred_polygons: min_area_polygons of 9 points

        Returns:
            centerness quality of predicted polygons

        """
        pos_pred_polygons = pos_pred_polygons.reshape(-1, 4, 2)
        point0 = pos_pred_polygons[:, 0, :].reshape(-1, 2)
        point1 = pos_pred_polygons[:, 1, :].reshape(-1, 2)
        point2 = pos_pred_polygons[:, 2, :].reshape(-1, 2)
        point3 = pos_pred_polygons[:, 3, :].reshape(-1, 2)
        a = perpendicular_dist(point0, point1, pos_center)
        b = perpendicular_dist(point1, point2, pos_center)
        c = perpendicular_dist(point2, point3, pos_center)
        d = perpendicular_dist(point3, point0, pos_center)
        centerness = torch.pow((torch.min(a, c) / torch.max(a, c)) * (torch.min(b, d) / torch.max(b, d)),
                               1 / self.centerness_alpha)
        return 1.0 - centerness
    ...
    def pointsets_quality_assessment(self, pts_features, cls_score,
        ...
        pos_center = center[pos_inds, 0:2]
        ...
        # quality of centerness
        qua_centerness_init = self.centerness_qua_weight * self.centerness_quality(pos_center, polygons_pred_init)
        qua_centerness_refine = self.centerness_qua_weight * self.centerness_quality(pos_center, polygons_pred_refine)
        ...
        qua = qua_cls + self.init_qua_weight * (
                qua_loc_init + qua_ori_init + qua_centerness_init) + (1.0 - self.init_qua_weight) * (
                      qua_loc_refine + qua_ori_refine + qua_centerness_refine) + qua_poc
    ...
    def dynamic_pointset_samples_selection(self,
        ...
        for gt_ind in range(num_gt):
            ...
            # 对每一个样本选择quality分数最小的min(level_gt_mask.sum(), 6)个样本
    ...
    def _point_target_single(self,
        ...
        # 样本的分配情况，0代表未分配，其余分配样本以1开始为索引进行编号
        gt_inds = assign_result.gt_inds

        # 对输出进行初始化
        num_valid_proposals = proposals.shape[0]
        # 输出的边框信息：4个点的标注，8维 (DOTA数据集)
    ...
    def loss(self,
        ...
        featmap_sizes = [featmap.size()[-2:] for featmap in cls_scores]  # 五个不同尺度的feature map 8-128，channel=15
        assert len(featmap_sizes) == self.prior_generator.num_levels
        label_channels = self.cls_out_channels if self.use_sigmoid_cls else 1
        device = cls_scores[0].device

        # 为每个特征图生成中心点的坐标以及valid标志
        center_list, valid_flag_list = self.get_points(
            featmap_sizes, img_metas, device=device)

        # 根据生成的中心点坐标以及一阶段预测的点的偏移，生成一阶段预测点的实际坐标
        pts_coordinate_preds_init = self.offset_to_pts(center_list,
                                                       pts_preds_init)

        # 计算每个尺度特征图生成的proposal的数量
        ...
        # center_img
        center_img = [
            torch.cat(item) for item in center_list
        ]
        ...
        with torch.no_grad():
            ...
            quality_assess_list, = multi_apply(
                ...
                bbox_weights_list_refine, pos_inds_list_refine, center_img)
        ...
        pos_inds_list_refine = [((0 <= labels) &
                                 (labels < self.num_classes)).nonzero(
            as_tuple=False).reshape(-1) for labels in labels_list]
        ...
        if num_pos:
            ...
            # repulsion loss
            loss_attraction_list, loss_regGT_list, loss_repBox_list = multi_apply(self.repulsion_loss,
                                                                                  pts_coordinate_preds_refine_img,
                                                                                  bbox_gt_list_refine,
                                                                                  pos_inds_list_refine,
                                                                                  labels_list
                                                                                  )
            loss_attraction = torch.as_tensor(loss_attraction_list).type_as(loss_border_refine).mean()
            loss_regGT = torch.as_tensor(loss_regGT_list).type_as(loss_border_refine).mean()
            loss_repBox = torch.as_tensor(loss_repBox_list).type_as(loss_border_refine).mean()
            ...
            loss_attraction, loss_regGT, loss_repBox = cls_scores.sum() * 0
        ...
        loss_dict_all = {
            ...
            'loss_regGT': loss_regGT,
            'loss_repBox': loss_repBox,
        }
