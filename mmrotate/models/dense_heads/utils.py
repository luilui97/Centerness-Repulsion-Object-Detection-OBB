...
def perpendicular_dist(point0, point1, center_point):
    """
    compute perpendicular dist of two points with center_point
    refers to https://github.com/steven-lang/dafne
    Args:
        point0: (N,2)->x,y
        point1: (N,2)->x,y
        center_point: (N,2)->x,y

    Returns:
        perpendicular dist of two points with center_point
    """
    numerator = torch.abs((point1[:,0] - point0[:,0]) * (point0[:,1] - center_point[:,1]) - \
                          (point0[:,0] - center_point[:,0]) * (point1[:,1] - point0[:,1]))
    denominator = torch.sqrt(torch.pow(point1[:,0] - point0[:,0], 2) + \
                             torch.pow(point1[:,1] - point0[:,1], 2))
